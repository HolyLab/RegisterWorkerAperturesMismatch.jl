@testset "overall registration" begin
    for (sz, mxshift, gridsize, order, steps) in (
            ((150,), (6,), (4,), (:x, :time), (1, 1)),
            ((150, 140), (6, 5), (4, 3), (:x, :y, :time), (1, 1, 1)),
            ((150, 140, 15), (6, 5, 2), (4, 3, 3), (:x, :y, :z, :time), (1, 1, 1, 1)),
        )
        N = length(sz)
        nodes = ntuple(d -> range(1, stop = sz[d], length = gridsize[d]), N)
        ...
        for I in CartesianIndices(gridsize)
            c = [round(Int, nodes[d][I[d]]) for d in 1:N]
            fixed[c...] = 2
        end
        ...
        for t in 1:nt
            ...
            for I in CartesianIndices(gridsize)
                c = [round(Int, nodes[d][I[d]]) + disp[d][I] + offset[d] for d in 1:N]
                moving_full[c..., t] = 2
            end
        end
        ...
        algorithm = AperturesMismatch[AperturesMismatch(fixed, nodes, mxshift; tid = wtids[i], correctbias = false) for i in 1:length(wtids)]
        ...
        ### Read the mismatch
        Es, cs, Qs, nodes, mmis = jldopen(fnmm, mmaparrays = true) do file
            read(file, "Es"), read(file, "cs"), read(file, "Qs"), read(file, "nodes"), read(file, "mmis")
        end
        ...
        for i in CartesianIndices(size(den)[(N + 1):end])
            ...
            @test (1 + 1.0e-8) * minimum(den1) > maximum(den1)
        end

        # Test that a perfect match was found in each aperture
        @test all(Es .< 1.0e-12) # if this fails, make sure checkbias=false above
